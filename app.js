...
function openMapPopup(targetId, spot, keepHighlight=false){
  ...
  const isTopLeftSpot = [1,2,3,4,5,8,10].includes(Number(spot));
  const useBelow = !isTopLeftSpot && y < 24;
  popup.classList.toggle('below', useBelow);
  popup.classList.toggle('spot-top-left', isTopLeftSpot);
  popup.classList.toggle('left-edge', !isTopLeftSpot && x < 24);
  popup.classList.toggle('right-edge', !isTopLeftSpot && x > 76);
  const left = isTopLeftSpot ? x : (x < 24 ? Math.min(x + 5, 24) : x > 76 ? Math.max(x - 5, 76) : x);
  const top = isTopLeftSpot ? Math.max(y - 3.8, 6) : (useBelow ? Math.min(y + 7, 92) : Math.max(y - 2, 8));
